...
<div class="section-body">
          ...
          <div class="card-body">
            <table class="table" id="mytable">
                ...
                <tbody>
                    ...
                    $sr=1;
                @endphp
                    @foreach ($IPModel as $item)
                   
                    <tr>
                        <td scope="row">{{ $sr++ }}</td>
                      
                          <td class="IP_id" style="display: none">{{ $item->id }}</td>
                       
                        <td class="IPNumber">{{ $item->YourIP }}</td>
                        ...
                        <td> <a href="{{ route('admin.DeleteIP', ['id' => $item->id]) }}" onclick="return confirm('Are you sure to delete this record')" class="btn btn-danger btn-sm">Delete</a></td>
    ...
    $('#IPNumber').val(IPNumber);
      ...
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalCenterTitle">Add IP</h5>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <form action="{{ route('admin.IPAdd') }}" method="post">
            @csrf
        <div class="modal-body">
